...
def rename_assets(search_pattern, replace_pattern, use_case):
    ...
    editor_util = unreal.EditorUtilityLibrary() #  - this will be used to get the selected assets.
    string_lib = unreal.StringLibrary() 

    # get the selected assets
    selected_assets = editor_util.get_selected_assets() # - get the currently selected assets
    num_assets = len(selected_assets) # - the length of our asset list
    replaced = 0    # - need to keep track of number of assets replaced. 

    unreal.log("Selected {} assets".format(num_assets))

    # loop over the assets and rename
